        ...
        await dexieDB.transaction('rw', Object.keys(data.collections), async () => {
            for (const collectionName in data.collections) {
                ...
                if (table) {
                    ...
                    // Validação e mapeamento para garantir ID único
                    const validatedData = collectionData.map((item: any, index: number) => {
                        if (!item.id) {
                            console.warn(
                                `Item na coleção '${collectionName}' sem ID. Gerando UUID:`,
                                item
                            );
                    ...
                    // Limpa antes de inserir (se for "sync full")
                    await table.clear();
                    console.log(`Tabela local '${collectionName}' limpa.`);

                    // Usa bulkPut (mais seguro que bulkAdd)
